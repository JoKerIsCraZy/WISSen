...
async function refreshDiag() {
  ...
  if (info.hasSW) {
    try {
      const reg = await navigator.serviceWorker.getRegistration('/pwa-demo/mobile/');
      if (reg) {
...
async function enablePush() {
  ...
  let reg = await navigator.serviceWorker.getRegistration('/pwa-demo/mobile/');
  if (!reg) {
    try {
      reg = await navigator.serviceWorker.register('/pwa-demo/mobile/sw.js', { scope: '/pwa-demo/mobile/' });
    } catch (e) {
